...
export default function UploaderAliPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerAliPay?.qrcodeImage
              })
            );
        }
    };
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerAliPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
