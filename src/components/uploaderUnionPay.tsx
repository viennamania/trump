...
export default function UploaderUnionPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerUnionPay?.qrcodeImage
              })
            );
        }
    };
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerUnionPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
