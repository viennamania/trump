...
export default function UploaderNaverPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerNaverPay?.qrcodeImage
              })
            );
        }
    };
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerNaverPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
