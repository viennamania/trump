...
export default function UploaderKakaoPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerKakaoPay?.qrcodeImage
              })
            );
        }
    };
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerKakaoPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
