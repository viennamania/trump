...
export default function UploaderWechatPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerWechatPay?.qrcodeImage
              })
            );
        }
    };
  ...
  return (
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerWechatPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
    ...
    >
      <div>
        ...
        <label
          htmlFor="image-upload-wechatpay"
