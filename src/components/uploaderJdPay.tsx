...
export default function UploaderJdPay(
    ...
    const fetchData = async () => {
        ...
        if (data.result) {
            setData(
              (prev) => ({
                ...prev,
                image: data.result?.sellerJdPay?.qrcodeImage
              })
            );
        }
    };
      ...
      onSubmit={async (e) => {
        ...
        fetch('/api/upload', {
          ...
          if (res.status === 200) {
            ...
            const result = await fetch("/api/user/updateSellerJdPay", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
